...
public class HeaderLevel : Element {
    ...
    public HeaderLevel(HeaderLevel level, string text) {
        PrivateTag = level.PrivateTag;
        PrivateText = text;
    }

    private HeaderLevel(string tag) {
        PrivateTag = tag;
    }
    ...
    public override string ToString() {
        var tag = new HtmlTag(PrivateTag).SetValue(PrivateText);
